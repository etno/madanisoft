<?php
	...
	if (!function_exists('get_user_groups')) {
		function get_user_groups() {
			...
			$value = $CI->m_user->get_group_list();
			return $value;
		}
	}

	if (!function_exists('get_user_levels')) {
		function get_user_levels() {
			$CI = & get_instance();
			$CI->load->model('m_user');
			$value = $CI->m_user->get_job_level();
			return $value;
		}
	}

	if (!function_exists('get_room_types')) {
		function get_room_types($status="") {
			$CI = & get_instance();
			$CI->load->model('m_room');
			if(empty($status)){
				$value = $CI->m_room->get_type();
			}else{
				$value = $CI->m_room->get_type(0,0,0,array("status"=>$status));
			}
			return $value;
		}
	}
